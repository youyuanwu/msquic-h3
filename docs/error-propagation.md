...
9. **Provenance selection and docs.rs (SF-L).** Native-library provenance is a
   ...
   registered via `[lints.rust] check-cfg` so `-D warnings` stays clean).
   Selecting **neither** provenance is a supported type-check-only configuration
   (the crate compiles without linking a native library — exercised by the
   default-features CI `cargo check`/`clippy` job; a real build/link requires a
   provenance), so it is deliberately NOT guarded by a `compile_error!`. The
